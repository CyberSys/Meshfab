...
void Mesh::create()      // by Xi Yang
{
	...
	glBufferData(GL_ARRAY_BUFFER, VERT_COUNT * sizeof(glm::vec4), obj_vert_col.data(), GL_DYNAMIC_DRAW);
...
void Mesh::create_wireframe()
{
	...
	for (unsigned int i = 0; i < vertices.size(); i++)
	{
		...
		vert_pos.push_back(vert->GetPos());
	}

	for (size_t i = 0; i < edges.size(); ++i)
	{
		auto vert = edges[i]->GetVertex()->GetPos();

		for (size_t j = 0; j < vert_pos.size(); ++j)
		{
			if (vert_pos[j] == vert)
				indx.push_back(j);
		}
